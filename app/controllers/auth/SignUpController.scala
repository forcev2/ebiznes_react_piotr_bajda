...
class SignUpController @Inject()(components: DefaultSilhouetteControllerComponents, userRepository: UserRepository, authTokenRepository: AuthTokenRepository)(implicit ex: ExecutionContext) extends SilhouetteController(components) {
  ...
  def signUp: Action[AnyContent] = unsecuredAction.async { implicit request: Request[AnyContent] =>
    ...
    userRepository.retrieve(loginInfo).flatMap {
      case Some(_) =>
        Future.successful(Forbidden(Json.toJson("Already email exists")))
      case None =>
