...
@Singleton
class SignInController @Inject()(scc: DefaultSilhouetteControllerComponents, addToken: CSRFAddToken, userRepository: UserRepository)(implicit ex: ExecutionContext) extends AbstractAuthController(scc) {
  ...
  def signIn: Action[AnyContent] = addToken(unsecuredAction.async { implicit request: Request[AnyContent] =>
    ...
    credentialsProvider.authenticate(credentials).flatMap { loginInfo =>
      userRepository.retrieve(loginInfo).flatMap {
        case Some(user) =>
          authenticateUser(user, name, value)
            .map(_.withCookies(Cookie(name, value, httpOnly = false)))
        case None => Future.failed(new IdentityNotFoundException("Couldn't find user"))
      }
    }.recover {
      case _: ProviderException =>
        Forbidden(Json.toJson("Provider Exception"))
